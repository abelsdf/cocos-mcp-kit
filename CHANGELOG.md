...
Changes to Cocos MCP Kit are documented above the preserved upstream Funplay history below.
...
- Hardened `delete_asset` for OP-071 across imported JSON, text, image, and audio main assets. It now checks main/subasset native references, ignores only same-main internal subasset links, rechecks identity, bytes and metadata after reference queries, sends one asset-db delete request, and confirms UUID/URL records, mappings, source, and `.meta` removal. Unsupported identities, external references, uncertain writes, force/cascade behavior, and filesystem fallback are refused.
- Added full-profile `move_asset` for OP-070. It moves or renames imported JSON, text, image, and audio main assets through one Cocos asset-db request, refuses overwrite, extension changes, case-only renames, subassets, and unsupported identities, and verifies removed source paths, exact bytes, preserved main/subasset UUIDs, importer settings, database readiness, and settled import state. UUID references remain valid; dynamic string/path references are reported as an explicit limitation.
- Added full-profile `copy_asset` for OP-069. It copies imported JSON, text, image, and audio main assets through one Cocos asset-db request, refuses overwrite and unsupported identities, and verifies exact bytes, copied importer settings, distinct main/subasset UUIDs, unchanged source files, database readiness, and settled import state.
- Added full-profile `create_asset` for OP-068, limited to new UTF-8 JSON/text assets in existing project asset directories. Creation goes through Cocos asset-db and verifies exact source content, `.meta` UUID/importer, imported type, readiness, and settled identity; overwrite, serialized Cocos formats, scripts, binaries, symlinked paths, and uncertain automatic retries are refused.
- Expanded `inspect_asset` for OP-067 with a stable compact `details` projection covering target/source identity, project-relative source files, disk availability and size, target/source/metadata importers, metadata ownership and user-data keys, and explicit main/subasset linkage. Missing source files now make the inspection incomplete.
...
- Added full-profile `exit_prefab_edit_mode` with explicit prefab/origin UUIDs, dirty and serialized-content guards, one native close, and double verification of the restored scene and unchanged files. Repeats in the matching verified scene never close it; the returned `needsSave` preserves the origin's actual dirty state. No automatic save, discard, retry, reopen or rollback.
- Added full-profile `save_prefab_edit_mode` for property-only native prefab saves, using the explicit current asset UUID and source hash returned by entry or the last verified save. Rejects source conflicts and unsupported structure/references, handles bounded target reimport, verifies source/live/origin state, and makes clean unchanged repeats no-ops. Never automatically exits, retries writes, rolls back or saves the origin scene.
- Added full-profile `enter_prefab_edit_mode` with exact asset selection, clean saved-scene and serialized-content guards, one native open, and verified edit-root/source/origin identity. Repeated entry into the same clean prefab does not reload it; dirty, multi-scene, nested and other-prefab states are refused, with no automatic save, discard, exit or retry.
- Hardened `revert_prefab_instance` with explicit non-nested root selection, property-only/reference guards and one verified native `restore-prefab` call. Checks restored serialized values, stable live identities and unchanged source files; preserves root name/position/rotation while restoring scale. Requires explicit scene save, with no guessed-message retry or automatic rollback.
- Hardened `apply_prefab_instance` with exact non-nested root selection, source/metadata identity and serialized-reference preflight, one native apply, and verified serialized source writeback. Refuses hierarchy/component changes and outgoing external scene references; native return values are not treated as success evidence, and uncertain writes have no retry or rollback fallback.
- Added `unlink_prefab_instance` with exact-root selection, saved-scene guards, a single native unlink and structural/link-removal verification. Non-nested UI, non-UI and script/Button-reference fixtures passed Creator 3.8.8 save/reopen and editor restart; nested hierarchies are explicitly refused after a cross-instance component-reference persistence failure was reproduced.
...
The entries from 0.6.3 downward describe the inherited Funplay codebase and are retained as upstream history.
