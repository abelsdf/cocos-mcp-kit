...
const ZH_TOOL_DESCRIPTIONS = {
  ...
  save_asset: '通过 Cocos asset-db 安全保存已有 JSON 或纯文本主资源；可用源 SHA-256 防止并发覆盖，并核对内容、UUID、导入器、元数据和稳定导入状态。',
  reimport_asset: '通过 Cocos asset-db 重新导入已有 JSON、文本、图片或音频主资源，并核对源文件、UUID、导入设置、子资源和稳定就绪状态。',
  ...
  query_asset_path: '按精确 UUID、db URL 或工程路径查询资源的真实源路径；子资源会区分所属主资源文件与不可直接读写的原生映射路径。',
  query_asset_uuid: '按精确 db URL、工程路径或 UUID 查询已导入资源的 UUID，并核对原生映射及子资源与主资源的关系。',
  query_asset_url: '按精确 UUID、db URL 或工程路径查询资源的规范 URL；原生子资源 @ 别名会单独核对并标注。',
  ...
  write_file: '通过文件系统写入或覆盖工程文件；该操作不证明 Cocos 资源已可靠持久化，资源应使用 save_asset 或对应类型的保存流程。',
  replace_in_file: '通过文件系统替换工程文件文本，适合脚本自动修复循环；该操作不证明 Cocos 资源已可靠持久化。',
