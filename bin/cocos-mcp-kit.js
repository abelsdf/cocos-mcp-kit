...
async function main() {
  ...
  if (hasFlag(args, '--version')) {
    console.error(`cocos-mcp-kit ${readPackageVersion()}`);
    return 0;
  }

  const urlText = getOption(args, '--url')
    || process.env.COCOS_MCP_KIT_URL
    || DEFAULT_URL;
  const timeoutText = getOption(args, '--timeout-seconds')
    || process.env.COCOS_MCP_KIT_TIMEOUT_SECONDS
  ...
  console.error(`[Cocos MCP Kit] Bridging stdio to ${endpoint.href}`);
...
async function bridgeStdioToHttp({ input, output, endpoint, timeoutMs }) {
  ...
  while (true) {
    ...
    try {
      ...
      console.error(`[Cocos MCP Kit] ${error.message}`);
      if (requestId !== null) {
...
function printHelp() {
  ...
  console.error('  COCOS_MCP_KIT_URL');
  console.error('  COCOS_MCP_KIT_TIMEOUT_SECONDS');
}
