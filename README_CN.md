...
| 领域 | 已提供的能力 | 示例工具 |
|---|---|---|
| 工程与资源 | 查询编辑器、场景、精确资源元数据/数据、真实源路径及 asset-db 就绪状态，安全创建或保存 JSON/文本资源、导入有界的外部文件或目录、复制、移动、刷新或重导入受支持资源，并检查引用、日志和脚本诊断。 | `get_project_info`、`inspect_asset`、`query_asset_path`、`check_asset_ready`、`create_asset`、`save_asset`、`import_asset`、`import_folder`、`copy_asset`、`move_asset`、`refresh_asset`、`reimport_asset`、`list_assets` |
