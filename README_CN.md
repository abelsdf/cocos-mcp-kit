...
`enter_prefab_edit_mode`（`full` 配置）从单个干净且已保存的场景进入明确指定的非嵌套工程预制体原生编辑模式。即使脏标记为 false，也会比较实时序列化与磁盘；核验实际编辑模式、预制体根身份和源资源/原场景文件未变。同一干净预制体重复调用只核验、不重载；拒绝脏状态、多场景及正在编辑其他预制体，不自动保存、丢弃或退出。编辑态保存与退出仍是独立待开发项，通用 `open_asset` 不具备这些专项保护。详见[进入编辑模式验收与限制](./docs/verification/PREFAB_EDIT_ENTER_2026-09-22.md)。
