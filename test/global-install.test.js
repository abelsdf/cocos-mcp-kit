...
test('global install state targets the active Creator version directory', () => {
  ...
  try {
    ...
    const packagePath = path.join(projectPath, 'extensions', 'cocos-mcp-kit');
    ...
    assert.equal(
      ...
      path.join(editorHomePath, 'builtin-extensions', '3.8.8', 'cocos-mcp-kit')
    );
  ...
  const editor = {
    ...
    Package: {
      ...
      async register(packagePath) {
        packages.push({ name: 'cocos-mcp-kit', path: packagePath, enable: false });
      },
...
test('global install state detects updates and duplicate project copies', () => {
  ...
  try {
    const homePath = path.join(temp, 'home');
    const projectPath = path.join(temp, 'project');
    const projectPackagePath = path.join(projectPath, 'extensions', 'cocos-mcp-kit');
...
test('installGlobalExtension verifies and installs a release into a new global path', {
  ...
  try {
    ...
    const releasePackagePath = path.join(releaseRoot, 'cocos-mcp-kit');
    const zipName = 'CocosMcpKit.v0.4.6.zip';
