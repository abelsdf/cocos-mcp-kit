...
test('createCocosMcpProjectSkill writes the recommended MCP workflow skill', () => {
  ...
  assert.equal(result.path, '.agents/skills/cocos-mcp-kit-workflow/SKILL.md');
  const content = readProjectInstruction(projectPath, result.path).content;
  assert.match(content, /Cocos MCP Kit Workflow/);
...
test('createCocosUiProjectSkill writes the responsive Cocos UI composition skill', () => {
  ...
  assert.equal(result.path, '.agents/skills/cocos-mcp-kit-ui-composition/SKILL.md');
  const content = readProjectInstruction(projectPath, result.path).content;
  assert.match(content, /^---\nname: cocos-mcp-kit-ui-composition\n/);
