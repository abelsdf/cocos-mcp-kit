...
function createReleasePackage(root, version) {
  const packageRoot = path.join(root, 'cocos-mcp-kit');
...
test('parseChecksum returns the checksum for a named release asset', () => {
  ...
  assert.equal(parseChecksum(`${checksum}  CocosMcpKit.v0.4.1.zip\n`, 'CocosMcpKit.v0.4.1.zip'), checksum);
...
test('extractZip extracts the extension package safely', { skip: hasCommand('zip') ? false : 'zip command unavailable' }, () => {
  ...
  try {
    ...
    assert.equal(fs.existsSync(path.join(extractRoot, 'cocos-mcp-kit', 'package.json')), true);
    assert.equal(fs.existsSync(path.join(extractRoot, 'cocos-mcp-kit', 'browser.js')), true);
  } finally {
...
test('installLatestUpdate downloads, verifies, backs up, and replaces the extension package', {
  ...
  let server = null;
  try {
    const currentPackage = path.join(temp, 'extensions', 'cocos-mcp-kit');
    ...
    const zipName = 'CocosMcpKit.v0.4.1.zip';
