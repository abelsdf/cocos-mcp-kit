...
`enter_prefab_edit_mode` (`full` profile) enters native editing for an exact non-nested project prefab from one clean, saved scene. It compares live serialization with disk even when the dirty flag is clear, verifies the actual editor mode and prefab root, and checks that source/origin files remain unchanged. Re-entering the same clean prefab verifies it without reloading; dirty, multi-scene and other-prefab states are refused. It never auto-saves, discards or exits. Saving and exiting prefab editing remain separate, unfinished operations; the generic `open_asset` does not provide these guards. See the [entry verification and limits](./docs/verification/PREFAB_EDIT_ENTER_2026-09-22.md).
