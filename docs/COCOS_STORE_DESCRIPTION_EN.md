# Cocos MCP Kit (unpublished Store copy draft)

## 【Feature Introduction】

Cocos MCP Kit is an open-source AI development extension for **Cocos Creator 3.8 and later**. It embeds an MCP Server directly in the Cocos Creator editor, allowing MCP-compatible coding assistants such as Claude Code, Cursor, Codex, VS Code Copilot, Trae, Kiro, Qoder, and Kimi Code to connect to the active project, read real editor context, and execute development workflows.

Unlike an assistant that can only analyze project files, Cocos MCP Kit can inspect the active scene, node hierarchy, components, assets, prefabs, logs, script diagnostics, editor state, and screenshots. This lets AI work from the project's actual state and helps reduce repeated switching between the editor, terminal, and AI client.
...
- **Release channel pending**: This fork does not yet have its own release source; automatic update and one-click installation are not in use.
...
6. **Confirm the connection**
   ...
   Return to the AI client and confirm that `cocos-mcp-kit` is connected and its tools are available. For a first test, call `get_project_info`, `get_scene_info`, or `get_hierarchy` to inspect the active project and scene.
...
7. **Start working**
   ...
   > Create a settings UI with a title, sound toggle, and close button in the active scene, then capture a screenshot to verify it.

   > Switch to Browser Preview, launch the active scene, and return the preview URL.

8. **Adjust tool exposure when needed**

   The default `core` profile is suitable for common inspection and development tasks. For file writes, complete scene editing, preview switching, and other advanced operations, open `Cocos MCP Kit > Tool Exposure` and select `full` or create a custom profile.

9. **Check for updates**

   This fork has no release source configured yet. Update it from the local repository and avoid installing upstream release packages through the panel.

## 【Contact】

**Open-source repository**

Add the project-owned repository after it exists.

**Issue reports and feature requests**

Add the project-owned issue tracker after it exists.

**Author**

Cocos MCP Kit contributors (based on Funplay open-source code)

**Contact email**

To be supplied by the project maintainer.
