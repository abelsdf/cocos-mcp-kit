...
const PACKAGE_NAME = 'cocos-mcp-kit';
...
function requestUrl(url, options = {}, redirectCount = 0) {
  return new Promise((resolve, reject) => {
    ...
    const request = client.get(
      ...
      {
        headers: {
          'User-Agent': 'cocos-mcp-kit-updater',
          Accept: '*/*',
        },
...
async function installLatestUpdate(options = {}) {
  ...
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'cocos-mcp-kit-update-'));
  ...
  try {
    ...
    if (packagePathExisted) {
      const backupRoot = path.join(path.dirname(packagePath), '.cocos-mcp-kit-backups');
