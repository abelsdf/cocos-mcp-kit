...
const { refreshAsset } = require('../asset-refresh');
...
async function refreshAssets(projectPath, targetPath) {
  ...
  if (isPathInside(path.join(projectPath, 'assets'), targetPath)) {
    ...
    try {
      ...
      return `File written, but exact asset refresh failed for ${dbUrl}: ${error.message}`;
    }
  }
...
function createFileTools({
  ...
  return [
    ...
    },
    {
      name: 'refresh_asset',
      profile: 'full',
      description: '[specialist] Refresh one existing project JSON, text, image, or audio file through Cocos asset-db; verify source bytes, main/subasset identities, metadata, library outputs, readiness, and settled state. No root or directory fallback.',
      inputSchema: createSchema(
        {
          target: { type: 'string', description: 'Exact db://assets URL, assets path, or absolute file path of a supported regular file inside project assets (maximum 64 MiB).' },
        },
        ['target']
      ),
      handler: async (args) => {
        const { projectPath } = getRuntimeContext();
        return await refreshAssetImpl(projectPath, args);
      },
    },
    {
      name: 'refresh_assets',
