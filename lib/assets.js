...
const SAFE_DELETE_EXTENSIONS_BY_TYPE = Object.freeze({
  'cc.JsonAsset': new Set(['.json']),
  'cc.TextAsset': new Set(['.txt']),
  'cc.ImageAsset': new Set(['.png', '.jpg', '.jpeg', '.webp']),
  'cc.AudioClip': new Set(['.mp3', '.wav', '.ogg', '.m4a']),
});
const MAX_DELETE_SUBASSETS = 64;
...
function normalizeAssetInspectionTarget(rawTarget, projectPath) {
  ...
  if (forward.startsWith('db://') || forward === 'assets' ||
      forward.startsWith('assets/') || forward.startsWith('/assets/')) {
    const isDbUrl = forward.startsWith('db://');
    const normalized = isDbUrl ? forward : `db://${forward.replace(/^\//, '')}`;
    const segments = normalized.slice('db://'.length).split('/');
    if (segments.some((segment) => segment === '.' || segment === '..')) {
      throw new Error('Asset db URL must not contain traversal segments.');
    }
    target = normalized;
    kind = isDbUrl ? 'db-url' : 'project-path';
...
function summarizeAssetInfo(info) {
  ...
  return {
    ...
    library: typeof info.library === 'string' ? info.library : '',
...
function assetScope(url) {
  if (typeof url !== 'string') return 'unknown';
  if (url === 'db://assets' || url.startsWith('db://assets/')) return 'project';
  if (url === 'db://internal' || url.startsWith('db://internal/')) return 'internal';
  return url.startsWith('db://') ? 'database' : 'unknown';
}

function inspectAssetSourceFile(file, projectPath) {
  const result = {
  ...
  if (projectPath && path.isAbsolute(result.path)) {
    try {
      const projectRoot = path.resolve(projectPath);
      const absolute = path.resolve(result.path);
      if (isPathInside(projectRoot, absolute)) {
        result.scope = 'project';
        result.projectRelativePath = path.relative(projectRoot, absolute).replace(/\\/g, '/');
      } else {
  ...
  try {
    ...
    result.exists = true;
    result.kind = stat.isDirectory() ? 'directory' : stat.isFile() ? 'file' : stat.isSymbolicLink() ? 'symbolic_link' : 'other';
    result.sizeBytes = stat.isFile() ? stat.size : null;
    result.modifiedTimeMs = Number.isFinite(stat.mtimeMs) ? stat.mtimeMs : null;
  } catch (error) {
    result.status = error && error.code === 'ENOENT' ? 'missing' : 'error';
    result.exists = false;
...
function buildAssetDetails(info, relationship, metadataQuery, projectPath, maxItems) {
  const target = summarizeAssetInfo(info);
  const source = relationship.kind === 'subasset' && relationship.mainAsset
    ? { ...relationship.mainAsset }
    : { ...target };
  const metadata = metadataQuery.status === 'available' && metadataQuery.value && typeof metadataQuery.value === 'object'
    ? metadataQuery.value : null;
  const userData = metadata && metadata.userData && typeof metadata.userData === 'object'
    ? metadata.userData : null;
  const userDataKeys = userData ? Object.keys(userData).sort() : [];
  const subasset = relationship.kind === 'subasset'
    ? relationship.subAssets.find((entry) => entry &&
      ((info.uuid && entry.uuid === info.uuid) || (info.url && entry.url === info.url)))
    : null;
  const imported = info.imported;
  const invalid = info.invalid;
  const importStatus = invalid === true ? 'invalid'
    : imported === false ? 'not_imported'
      : imported === true ? 'ready' : 'unknown';
  const sourceFile = inspectAssetSourceFile(source.file || info.file, projectPath);
  const sourceUrl = info.source || source.source || source.url || '';
  const metadataUuid = metadata && typeof metadata.uuid === 'string' ? metadata.uuid : '';
  const metadataImporter = metadata && typeof metadata.importer === 'string' ? metadata.importer : '';
  const sourceImporter = source.importer || metadataImporter || '';

  return {
    status: relationship.status === 'available' && metadataQuery.status === 'available' &&
      !['missing', 'error'].includes(sourceFile.status) ? 'available' : 'incomplete',
    complete: relationship.status === 'available' && metadataQuery.status === 'available' &&
      !['missing', 'error'].includes(sourceFile.status),
    target: {
      name: target.name,
    ...
    source: {
      ...
      sourceUrl,
      type: source.type,
      scope: assetScope(source.url || sourceUrl),
      importer: sourceImporter,
      file: sourceFile,
    },
    import: {
      status: importStatus,
      usable: importStatus === 'ready',
      targetImporter: target.importer,
      sourceImporter,
      metadataImporter,
      imported: target.imported,
      invalid: target.invalid,
    ...
    metadata: {
      status: metadataQuery.status,
      scope: metadataQuery.scope || '',
      target: metadataQuery.target || '',
      uuid: metadataUuid,
      matchesTarget: Boolean(metadataUuid && metadataUuid === target.uuid),
      matchesSource: Boolean(metadataUuid && metadataUuid === source.uuid),
      importer: metadataImporter,
      version: metadata && (typeof metadata.ver === 'string' || typeof metadata.ver === 'number') ? metadata.ver : null,
      userDataKeyCount: userDataKeys.length,
      userDataKeys: userDataKeys.slice(0, maxItems),
      userDataKeysTruncated: userDataKeys.length > maxItems,
    },
    relationship: {
      kind: relationship.kind,
      status: relationship.status,
      mainUuid: source.uuid,
      mainUrl: source.url,
      subAssetKey: subasset && typeof subasset.key === 'string' ? subasset.key : '',
      subAssetCount: relationship.subAssetCount,
      subAssetsTruncated: relationship.subAssetsTruncated,
    },
  };
}
...
async function inspectAsset(target, options = {}) {
  ...
  const details = buildAssetDetails(info, relationship, metadataQuery, options.projectPath, limits.maxItems);
  const truncation = {
...
function assertRealPathInsideAssets(assetsRoot, file, label) {
...
function assertDeletableRegularAsset(projectPath, info, metadata) {
  if (!projectPath) throw new Error('A project path is required to delete an asset safely.');
  if (!info || !SUPPORTED_COPY_TYPES.has(info.type) || info.imported !== true || info.invalid === true ||
      info.readonly === true || info.isDirectory === true || typeof info.uuid !== 'string' || !info.uuid ||
      info.uuid.includes('@') || typeof info.url !== 'string' || !info.url.startsWith('db://assets/') ||
      (info.source && info.source !== info.url)) {
    throw new Error('Safe deletion requires a writable, fully imported JSON, text, image, or audio main asset with a UUID.');
  }
  const file = resolveProjectFilePath(projectPath, info.url.slice('db://'.length));
  const assetsRoot = resolveProjectFilePath(projectPath, 'assets');
  if (!isPathInside(assetsRoot, file) || path.resolve(file) === path.resolve(assetsRoot) ||
      `db://${path.relative(projectPath, file).replace(/\\/g, '/')}` !== info.url ||
      (info.file && path.resolve(resolveProjectFilePath(projectPath, info.file)) !== path.resolve(file))) {
    throw new Error('Asset URL and source file must identify the same file inside project assets.');
  }
  const extension = path.extname(file).toLowerCase();
  const extensions = SAFE_DELETE_EXTENSIONS_BY_TYPE[info.type];
  if (!extensions || !extensions.has(extension)) {
    throw new Error(`The ${info.type} source extension is not supported by safe deletion.`);
  }
  const stat = assertRegularFile(file, 'Asset source');
  assertRealPathInsideAssets(assetsRoot, file, 'Asset source');
  if (stat.size > MAX_COPY_BYTES) {
    throw new Error(`Asset exceeds the ${MAX_COPY_BYTES}-byte safe deletion limit.`);
  }
  const metaFile = resolveProjectFilePath(projectPath, `${file}.meta`);
  assertRegularFile(metaFile, 'Asset metadata');
  assertRealPathInsideAssets(assetsRoot, metaFile, 'Asset metadata');
  const diskMetadata = JSON.parse(fs.readFileSync(metaFile, 'utf8'));
  const importer = String(info.importer || (metadata && metadata.importer) || diskMetadata.importer || '');
  if (!metadata || !importer || metadata.uuid !== info.uuid || metadata.importer !== importer ||
      diskMetadata.uuid !== info.uuid || diskMetadata.importer !== importer ||
      (info.importer && info.importer !== importer)) {
    throw new Error('Asset metadata UUID/importer does not match asset-db. No asset was deleted.');
  }
  const subAssets = subAssetIdentities(info);
  if (subAssets.length > MAX_DELETE_SUBASSETS ||
      subAssets.some((entry) => !entry.uuid || !entry.type || entry.uuid === info.uuid) ||
      new Set(subAssets.map((entry) => entry.uuid)).size !== subAssets.length) {
    throw new Error(`Asset subasset identities are invalid or exceed the ${MAX_DELETE_SUBASSETS}-item safe deletion limit.`);
  }
  return {
    file,
    metaFile,
    ...
    metaHash: hashFile(metaFile),
    subAssets,
    identities: [
      { uuid: info.uuid, type: info.type, role: 'main' },
      ...subAssets.map((entry) => ({ uuid: entry.uuid, type: entry.type, role: `subasset:${entry.key}` })),
    ],
  };
}

function assertRegularAssetUnchanged(previous, currentInfo, currentMetadata, projectPath) {
  if (!currentInfo || currentInfo.uuid !== previous.info.uuid || currentInfo.url !== previous.info.url ||
      currentInfo.type !== previous.info.type || currentInfo.importer !== previous.info.importer) {
    throw new Error('Asset identity changed during deletion preflight. No asset was deleted.');
  }
  const current = assertDeletableRegularAsset(projectPath, currentInfo, currentMetadata);
  if (current.sourceHash !== previous.sourceHash || current.metaHash !== previous.metaHash ||
      !isDeepStrictEqual(current.subAssets, previous.subAssets) ||
      !isDeepStrictEqual(currentMetadata, previous.metadata)) {
    throw new Error('Asset source, metadata, or subasset identities changed during deletion preflight. No asset was deleted.');
  }
  return current;
}

async function queryRegularAssetReferences(identities) {
  const internalUuids = new Set(identities.map((identity) => identity.uuid));
  const results = [];
  for (const identity of identities) {
    const assetUsers = await safeRequest('asset-db', 'query-asset-users', identity.uuid, 'all');
    const sceneNodes = await safeRequest('scene', 'query-nodes-by-asset-uuid', identity.uuid);
    for (const [label, refs] of [['asset users', assetUsers], ['scene nodes', sceneNodes]]) {
      if (!Array.isArray(refs) || refs.some((ref) => typeof ref !== 'string' || !ref)) {
        throw new Error(`Asset reference query returned invalid ${label} for ${identity.role} ${identity.uuid}. No asset was deleted.`);
      }
    }
    const internalAssetUsers = assetUsers.filter((user) => internalUuids.has(user));
    const externalAssetUsers = assetUsers.filter((user) => !internalUuids.has(user));
    results.push({ ...identity, assetUsers: externalAssetUsers, internalAssetUsers, sceneNodes });
  }
  const referenced = results.filter((entry) => entry.assetUsers.length || entry.sceneNodes.length);
  if (referenced.length) {
    const details = referenced.slice(0, 20).map((entry) =>
      `${entry.role} ${entry.uuid}: ${entry.assetUsers.length} asset/script user(s) [${entry.assetUsers.slice(0, 10).join(', ')}], ` +
      `${entry.sceneNodes.length} active-scene node(s) [${entry.sceneNodes.slice(0, 10).join(', ')}]`
    ).join('; ');
    throw new Error(`Asset is still referenced: ${details}. Remove references and save their assets before retrying. No asset was deleted.`);
  }
  return results;
}

async function verifyAssetDeletion(uuid, url, file, metaFile) {
  ...
  return {
    ...
    metaAbsent: fileAbsent(metaFile),
  };
}

async function deleteRegularAsset(info, options) {
  const { uuid, url } = info;
  ...
  const metadata = await safeRequest('asset-db', 'query-asset-meta', uuid);
  const source = assertDeletableRegularAsset(options.projectPath, info, metadata);
  if (await safeRequest('asset-db', 'query-ready') !== true ||
      await safeRequest('scene', 'query-is-ready') !== true) {
    throw new Error('Asset database and scene must be ready for asset deletion reference checks.');
  }
  const references = await queryRegularAssetReferences(source.identities);

  for (const target of [uuid, url]) {
    const currentInfo = await safeRequest('asset-db', 'query-asset-info', target);
    const currentMetadata = currentInfo && currentInfo.uuid
      ? await safeRequest('asset-db', 'query-asset-meta', currentInfo.uuid) : null;
    assertRegularAssetUnchanged({ ...source, info }, currentInfo, currentMetadata, options.projectPath);
  }

  try {
    await safeRequest('asset-db', 'delete-asset', uuid);
  } catch (error) {
    throw new Error(`asset-db:delete-asset failed for ${url}: ${error.message}. The result may be uncertain; inspect asset-db and both source paths before retrying.`, { cause: error });
  }

  let verification;
  try {
    for (let attempt = 0; attempt <= retries; attempt += 1) {
      verification = await verifyAssetDeletion(uuid, url, source.file, source.metaFile);
      if (Object.values(verification).every(Boolean)) {
        return {
          ...
          method: 'asset-db:delete-asset',
          referencePreflight: {
            identitiesChecked: references.length,
            assetUserCount: 0,
            sceneNodeCount: 0,
            internalAssetUserCount: references.reduce((count, entry) => count + entry.internalAssetUsers.length, 0),
          },
          verification,
          warnings: ['UUID reference checks do not discover dynamic string or path-based loading.'],
        };
      }
      if (attempt < retries) await new Promise((resolve) => setTimeout(resolve, retryDelayMs));
    }
  } catch (error) {
    throw new Error(`Asset deletion not confirmed: ${error.message}. Deletion may already have occurred; inspect asset-db and disk before retrying.`, { cause: error });
  }
  const incomplete = Object.keys(verification).filter((key) => !verification[key]);
  throw new Error(`Asset deletion not confirmed (${incomplete.join(', ')}). Inspect asset-db and disk before retrying; deletion was requested only once.`);
}
...
async function deleteAsset(uuidOrPath, options = {}) {
  ...
  if (target !== info.uuid && target !== info.url &&
      !(path.isAbsolute(target) && info.file && path.resolve(target) === path.resolve(info.file))) {
    throw new Error('Asset target must be an exact UUID, db URL, or source file path.');
  }
  if (info.type === 'cc.Prefab' || /\.prefab$/i.test(info.url) || /\.prefab$/i.test(target)) {
    return await deletePrefabAsset(info, options);
  }
  return await deleteRegularAsset(info, options);
}
