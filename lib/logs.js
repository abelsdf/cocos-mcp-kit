...
function describeReadError(error) {
  return error.message && error.message.startsWith(`${error.code}:`)
    ? error.message
    ...
    for (const entry of entries) {
      ...
      try {
        ...
        if (isUnreadableError(error)) {
          files.push({ fullPath, size: null, mtimeMs: 0, readError: describeReadError(error) });
        }
      }
...
function readTailResult(filePath, maxLines) {
  try {
    return { text: readTail(filePath, maxLines) };
  } catch (error) {
    if (isUnreadableError(error)) {
      return { text: '', readError: describeReadError(error) };
    }
    throw error;
  }
}
...
function getRecentProjectLogs(projectPath, options = {}) {
  ...
  return findProjectLogFiles(projectPath, options).map((file) => ({
    ...
    size: file.size,
    mtime: file.readError ? null : new Date(file.mtimeMs).toISOString(),
    ...(file.readError ? { text: '', readError: file.readError } : readTailResult(file.fullPath, maxLines)),
  }));
}
...
function searchProjectLogs(projectPath, options = {}) {
  ...
  for (const file of findProjectLogFiles(projectPath, { ...options, limit: normalizeLimit(options.fileLimit, 40, 1, 200) })) {
    const read = file.readError
      ? { text: '', readError: file.readError }
      : readTailResult(file.fullPath, normalizeLimit(options.linesPerFile, 2000, 1, 10000));
    if (read.readError) {
      skipped.push({ path: path.relative(projectPath, file.fullPath).replace(/\\/g, '/'), error: read.readError });
      continue;
    }
    ...
    for (let index = 0; index < lines.length; index += 1) {
      ...
      if (results.length >= limit) {
        return { query, count: results.length, matches: results, skipped };
      }
    }
  }
