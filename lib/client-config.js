...
const SERVER_NAME = 'cocos_mcp_kit';
const LEGACY_SERVER_NAME = 'funplay_cocos';
...
function getPreviousEntry(config, targetId) {
  ...
  return record && record.projectIdentity === getProjectIdentity(config.projectPath)
    && (record.serverName === SERVER_NAME || record.serverName === LEGACY_SERVER_NAME || /^cocos-[a-z0-9-]+$/.test(record.serverName)) ? record : undefined;
}
...
function configureJsonTarget(target) {
  ...
  if (target.migrateLegacy && name !== LEGACY_SERVER_NAME && isGeneratedEntry(servers[LEGACY_SERVER_NAME], target.entry.url)) delete servers[LEGACY_SERVER_NAME];
  if (target.scopePath && target.migrateLegacy && isObject(root.mcpServers) && isGeneratedEntry(root.mcpServers[LEGACY_SERVER_NAME], target.entry.url)) delete root.mcpServers[LEGACY_SERVER_NAME];
...
function configureTomlTarget(target) {
  ...
  const retired = getTomlBlocks(content).filter((block) => block.name !== name && (
    (target.migrateLegacy && block.name === LEGACY_SERVER_NAME && block.url === target.url) ||
