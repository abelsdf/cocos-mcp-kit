...
async function testPrefabEditMode(projectPath, sceneBridge, options = {}) {
  if (Object.keys(options).some(key => key !== 'prefabUuid')) throw new Error('Only the prefabUuid option is supported for read-only edit diagnostics.');
  if (typeof options.prefabUuid !== 'string' || !options.prefabUuid.trim()) throw new Error('An explicit non-empty prefab asset UUID is required.');
  const uuid = options.prefabUuid.trim();
  const editor = global.Editor;
  const report = {
    readOnly: true, editorVersion: editor && editor.App && typeof editor.App.version === 'string' ? editor.App.version : null,
    prefabUuid: uuid, complete: false, readChecksPassed: false, observationsStable: null,
    context: null, prefab: null, originScene: null, editing: null, checks: [],
    mutationTests: { enter: 'not_run', save: 'not_run', exit: 'not_run' },
  };
  const check = async (name, enabled, action, reason = 'A prerequisite is unavailable; this check was not run.') => {
    if (!enabled) { report.checks.push({ name, status: 'not_checked', reason }); return null; }
    try {
      const value = await action();
      report.checks.push({ name, status: 'passed' });
      return value;
    } catch (error) {
      report.checks.push({ name, status: 'failed', error: String(error && error.message || error).slice(0, 500) });
      return null;
    }
  };
  const checkReferences = async (content, output) => {
    const refs = await validateSerializedPrefabAssetReferences(content);
    output.references = { valid: refs.ok, complete: refs.complete, referenceCount: refs.referenceCount,
      totalReferenceCount: refs.totalReferenceCount, missingCount: refs.missingCount, lookupErrorCount: refs.lookupErrorCount };
    if (!refs.ok || !refs.complete) throw new Error(`Asset reference scan failed or incomplete: ${refs.missingCount} missing, ${refs.lookupErrorCount} lookup errors, ${refs.referenceCount}/${refs.totalReferenceCount} checked.`);
    return true;
  };
  const assetReady = await check('assetDatabase', true, async () => {
    if (await requestEditorMessage('asset-db', 'query-ready') !== true) throw new Error('Asset database is not ready.');
    return true;
  });
  const context = await check('editorContext', true, async () => {
    const current = await readPrefabEditContext();
    report.context = { ready: current.ready, mode: current.mode, uuid: current.uuid, dirty: current.dirty,
      multi: current.multi, tabCount: Array.isArray(current.tabs) ? current.tabs.length : null };
    assertPrefabEditContext(current, true);
    return current;
  });
  const source = await check('sourcePrefab', assetReady, async () => {
    const current = await readPrefabOperationSource(projectPath, uuid);
    const assetName = path.basename(current.file, '.prefab');
    const objects = JSON.parse(current.content);
    const metadata = assertSerializedPrefabMetadata(objects, { expectedName: assetName });
    const rootFileId = objects[objects[metadata.rootIndex]._prefab.__id__].fileId;
    const expected = normalizePrefabRevertContent(objects);
    report.prefab = { uuid, url: current.info.url, nodeCount: metadata.nodeCount, componentCount: metadata.componentCount, references: null };
    return { ...current, expected, assetName, rootFileId };
  });
  await check('sourceReferences', source, () => checkReferences(source.expected, report.prefab));
  const origin = await check('originScene', assetReady && context, async () => {
    const live = prefabOriginSceneContent(await requestEditorMessage('scene', 'query-scene-json'));
    const asset = live.find(entry => entry && entry.__type__ === 'cc.SceneAsset');
    const sceneUuid = live[asset.scene.__id__]._id;
    if (context.mode === 'general' && sceneUuid !== context.uuid) throw new Error('Current scene identity differs from the retained serialization.');
    const current = await readPrefabOperationSource(projectPath, sceneUuid, context.mode === 'general' ? context.tabs[0].url : undefined, true);
    report.originScene = { uuid: sceneUuid, url: current.info.url, dirty: context.mode === 'general' ? context.dirty : null,
      matchesSavedContent: isDeepStrictEqual(live, prefabOriginSceneContent(current.content, sceneUuid)) };
    return { ...current, live };
  });
  const targetOpen = Boolean(context && context.mode === 'prefab' && context.uuid === uuid);
  const readLive = async () => {
    if (context.tabs[0].url !== source.info.url) throw new Error('Current prefab tab does not match the source asset.');
    const state = await sceneBridge.call('getPrefabEditingState', { prefabUuid: uuid });
    const root = assertPrefabEditingRoot(state, source.info, source.assetName, source.rootFileId);
    const content = normalizePrefabRevertContent(await readPrefabApplyPreview(root.uuid, source.assetName));
    return { state, content };
  };
  const live = await check('editingRoot', targetOpen && source, async () => {
    const current = await readLive();
    const sourceMatchesLive = isDeepStrictEqual(source.expected, current.content);
    report.editing = { uuid: current.state.node.uuid, path: current.state.node.path,
      nodeCount: current.state.nodes.length, componentCount: current.state.nodes.reduce((sum, node) => sum + node.components.length, 0),
      sourceMatchesLive, structureMatchesSource: isDeepStrictEqual(prefabApplyStructure(source.expected), prefabApplyStructure(current.content)),
      hasUnsavedChanges: context.dirty || !sourceMatchesLive, references: null };
    return current;
  }, 'The target is not open in a supported prefab context, or its source is unavailable; no mode change was requested.');
  await check('liveReferences', live, () => checkReferences(live.content, report.editing));
  const canRecheck = context && source && origin && (!targetOpen || live);
  if (canRecheck) report.observationsStable = false;
  const stable = await check('stability', canRecheck, async () => {
    if (!isDeepStrictEqual(context, await readPrefabEditContext())) throw new Error('Editor context changed during diagnostics.');
    const current = await readPrefabOperationSource(projectPath, uuid, source.info.url);
    const retained = await readPrefabOperationSource(projectPath, origin.info.uuid, origin.info.url, true);
    if (await requestEditorMessage('asset-db', 'query-ready') !== true || current.content !== source.content || current.meta !== source.meta ||
        retained.content !== origin.content || retained.meta !== origin.meta ||
        !isDeepStrictEqual(origin.live, prefabOriginSceneContent(await requestEditorMessage('scene', 'query-scene-json'), origin.info.uuid)) ||
        live && !isDeepStrictEqual(live, await readLive())) {
      throw new Error('Prefab, retained scene or source files changed during diagnostics.');
    }
    return true;
  });
  if (stable) report.observationsStable = true;
  report.readChecksPassed = report.checks.every(item => item.status !== 'failed');
  report.complete = report.checks.every(item => item.status === 'passed');
  return report;
}
