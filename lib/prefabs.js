...
async function readPrefabOperationSource(projectPath, uuid, expectedUrl, sceneSource = false) {
  ...
  if (!info || info.uuid !== uuid || info.type !== type || info.imported !== true ||
...
function assertCleanPrefabEditContext(context) {
  if (context.ready !== true) throw new Error('Scene must be ready before entering prefab editing.');
  ...
  if (context.dirty !== false || context.multi !== false || !Array.isArray(context.tabs) || context.tabs.length !== 1 ||
      !context.tabs[0] || context.tabs[0].uuid !== context.uuid || context.tabs[0].dirty !== false ||
    ...
    throw new Error('Prefab entry requires one clean scene/tab, no dirty or multi-scene editing state. Save or resolve it explicitly first.');
  }
}
...
function prefabOriginSceneContent(content, uuid) {
  ...
  if (assets.length !== 1 || !root || root.__type__ !== 'cc.Scene' || root._id !== uuid) {
...
async function enterPrefabEditMode(projectPath, sceneBridge, options = {}) {
  ...
  assertCleanPrefabEditContext(before);
  ...
  assertCleanPrefabEditContext(rechecked);
  ...
  try {
    ...
    const verify = async () => {
      const context = await readPrefabEditContext();
      assertCleanPrefabEditContext(context);
