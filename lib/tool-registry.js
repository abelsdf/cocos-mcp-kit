...
function createToolRegistry({ getRuntimeContext, getStatus, interactionLog, runtimeLog, sceneBridge, editorExecutor }) {
  ...
  const tools = [
    ...
    },
    {
      ...
      profile: 'full',
      description: '[core] List bounded live component property snapshots on a scene node. Direct serialization metadata is advisory; save and reopen to verify persisted values.',
      inputSchema: createSchema(
        {
          path: { type: 'string', description: 'Node hierarchy path.' },
          uuid: { type: 'string', description: 'Node uuid.' },
