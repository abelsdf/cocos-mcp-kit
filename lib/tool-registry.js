...
function createToolRegistry({ getRuntimeContext, getStatus, interactionLog, runtimeLog, sceneBridge, editorExecutor }) {
  ...
  const tools = [
    ...
    },
    {
      name: 'inspect_asset',
      profile: 'core',
      description: '[specialist] Inspect one exact asset-db target with bounded info, metadata, optional serialized data, and explicit main/subasset relationships. Missing extensions are never guessed; query failures and truncation remain visible.',
      inputSchema: createSchema(
        {
    ...
    },
    {
      name: 'delete_asset',
      profile: 'full',
      description: 'Delete an exact asset by UUID, db URL, or path through asset-db. Project prefabs require no asset/script or active-scene references, and deletion is confirmed against both asset-db mappings and source/.meta files. Prefab deletion has no force or cascade option.',
      inputSchema: createSchema(
        {
          target: { type: 'string', description: 'Asset uuid, db url, or path.' },
        },
