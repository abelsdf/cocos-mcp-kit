...
const LOG_PREFIX = '[Cocos MCP Kit Server]';
...
class McpServer {
  ...
  async tryAttachToExisting(port) {
    ...
    if (!probe || !probe.result) {
      this.log('warn', `Port ${port} is occupied, but no compatible Cocos MCP Kit initialize response was received.`);
      return false;
    }
