...
const EXTENSION_ZIP_PATTERN = /^CocosMcpKit\.v.+\.zip$/i;
    ...
    const request = https.get(
      url,
      {
        headers: {
          ...
          'User-Agent': 'cocos-mcp-kit-update-checker',
        },
        timeout: timeoutMs,
      },
      (response) => {
...
function selectReleaseAssets(version, assets = []) {
  ...
  const exactZipName = `CocosMcpKit.v${normalizeVersion(version)}.zip`;
...
async function checkForUpdate(options = {}) {
  ...
  const releaseUrl = String(options.url || LATEST_RELEASE_URL).trim();
  ...
  if (!releaseUrl) {
    return {
      ...
      source: '',
      error: 'This fork has no release source configured yet.',
    };
  }

  try {
    const release = await fetchJson(releaseUrl, options.timeoutMs || 5000);
