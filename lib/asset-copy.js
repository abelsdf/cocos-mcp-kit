...
function normalizeCopyTarget(projectPath, target, operation = 'copy_asset') {
  ...
  if (!SUPPORTED_COPY_EXTENSIONS.has(extension)) {
    throw new Error(`Unsupported target extension. ${operation} supports JSON, text, image, and audio assets; use specialized workflows for scenes, prefabs, scripts, metadata, and other formats.`);
  }
