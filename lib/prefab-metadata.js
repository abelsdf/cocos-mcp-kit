...
function assertNoLinkedPrefabInstances(root) {
  const visited = new Set();
  function visit(node, path) {
    if (!node || typeof node !== 'object' || visited.has(node)) return;
    visited.add(node);
    const info = node._prefab;
    if (info && info.asset && info.instance) {
      throw new Error(`Cannot create a prefab from '${path}': nested linked prefab instances would be flattened. Create a regular node hierarchy or wait for nested prefab support.`);
    }
    for (const child of Array.isArray(node.children) ? node.children : []) {
      visit(child, `${path}/${child.name || 'Node'}`);
    }
  }
...
function assertSerializedPrefabMetadata(serialized, options = {}) {
  ...
  if (options.expectedName !== undefined && prefab._name !== options.expectedName) {
    invalidMetadata(`cc.Prefab._name is '${String(prefab._name || '')}', expected '${options.expectedName}'.`);
  }
  ...
  if (options.expectedName !== undefined && rootRef.value._name !== options.expectedName) {
    invalidMetadata(`prefab root _name is '${String(rootRef.value._name || '')}', expected '${options.expectedName}'.`);
  }
  ...
  for (const { value: node, index: nodeIndex } of nodes) {
    ...
    for (let componentOffset = 0; componentOffset < componentRefs.length; componentOffset += 1) {
      ...
      if (componentIds.has(componentRef.id)) {
        invalidMetadata(`component at index ${componentRef.id} is referenced by more than one node.`);
      }
      componentIds.add(componentRef.id);
      const componentOwner = componentRef.value.node || componentRef.value._node;
      if (referenceId(componentOwner, `component at index ${componentRef.id}.node`) !== nodeIndex) {
        invalidMetadata(`component at index ${componentRef.id} does not reference its owning node ${nodeIndex}.`);
      }
  ...
  function visitNode(nodeIndex, expectedParentIndex) {
    if (visitingNodes.has(nodeIndex)) {
      invalidMetadata(`node hierarchy contains a cycle at index ${nodeIndex}.`);
    }
    if (reachableNodes.has(nodeIndex)) {
      invalidMetadata(`cc.Node at index ${nodeIndex} is referenced by more than one parent.`);
    }
    const node = objects[nodeIndex];
    if (!node || node.__type__ !== 'cc.Node') {
      invalidMetadata(`hierarchy reference points to non-node object ${nodeIndex}.`);
    }
    if (expectedParentIndex === null) {
      if (node._parent && Number.isInteger(node._parent.__id__)) {
        invalidMetadata(`prefab root at index ${nodeIndex} unexpectedly has parent ${node._parent.__id__}.`);
      }
    } else if (referenceId(node._parent, `cc.Node at index ${nodeIndex}._parent`) !== expectedParentIndex) {
      invalidMetadata(`cc.Node at index ${nodeIndex} does not reference parent ${expectedParentIndex}.`);
    }
    visitingNodes.add(nodeIndex);
    for (let childOffset = 0; childOffset < (Array.isArray(node._children) ? node._children.length : 0); childOffset += 1) {
      const childRef = referencedObject(objects, node._children[childOffset],
        `cc.Node at index ${nodeIndex}._children[${childOffset}]`);
      visitNode(childRef.id, nodeIndex);
    }
    visitingNodes.delete(nodeIndex);
    reachableNodes.add(nodeIndex);
  }
  visitNode(rootRef.id, null);
  if (reachableNodes.size !== nodes.length) {
    const orphaned = nodes.filter(({ index }) => !reachableNodes.has(index)).map(({ index }) => index);
    invalidMetadata(`serialized node objects are outside the prefab root hierarchy: ${orphaned.join(', ')}.`);
  }

  for (let index = 0; index < objects.length; index += 1) {
    const value = objects[index];
    if (!value || value.__type__ === 'cc.Node') continue;
    const owner = value.node || value._node;
    if (owner && Number.isInteger(owner.__id__) && !componentIds.has(index)) {
      invalidMetadata(`component-like object at index ${index} is not referenced by its node._components list.`);
    }
  }

  let objectReferenceCount = 0;
  function checkObjectReferences(value, pointer = '') {
    if (!value || typeof value !== 'object') return;
    if (!Array.isArray(value) && Object.keys(value).length === 1 && Object.prototype.hasOwnProperty.call(value, '__id__')) {
      const id = value.__id__;
      if (!Number.isInteger(id) || id < 0 || id >= objects.length || !objects[id] || typeof objects[id] !== 'object') {
        invalidMetadata(`${pointer || 'object reference'} points to missing object ${String(id)}.`);
      }
      objectReferenceCount += 1;
      return;
    }
    if (Array.isArray(value)) {
      value.forEach((item, index) => checkObjectReferences(item, `${pointer}/${index}`));
    } else {
      for (const [key, child] of Object.entries(value)) {
        checkObjectReferences(child, `${pointer}/${key}`);
      }
    }
  }
  checkObjectReferences(objects);
