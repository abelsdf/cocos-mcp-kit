...
const COCOS_MCP_SKILL_NAME = 'cocos-mcp-kit-workflow';
const COCOS_MCP_SKILL_TITLE = 'Cocos MCP Kit Workflow';
const LEGACY_COCOS_MCP_SKILL_DESCRIPTION =
  'Use this skill when editing, validating, or debugging this Cocos Creator project through Cocos MCP Kit.';
const COCOS_MCP_SKILL_DESCRIPTION =
  'Edit, inspect, validate, preview, and debug Cocos Creator projects through Cocos MCP Kit. Use when working with scenes, nodes, prefabs, assets, TypeScript, logs, screenshots, preview behavior, runtime state, or MCP connectivity.';
const COCOS_MCP_SKILL_INSTRUCTIONS = [
  ...
  '   - Inspect the active scene with `get_scene_info` and `get_hierarchy`; use `get_selection`, `list_scenes`, `find_asset_by_name`, `list_assets`, or `list_prefabs` when identity or ownership is unclear.',
  ...
  '   - For `create_sprite`, use `spriteFrameTarget` for an imported image path, ImageAsset UUID, or exact SpriteFrame UUID; use `spriteFrameUuid` only for an already identified SpriteFrame subasset. Never pass a Texture2D UUID as a SpriteFrame.',
  '   - Use `set_sprite_frame` to change an existing Sprite reference; identify the node exactly, then save and reopen the scene or prefab to confirm the new subasset UUID persisted.',
  '   - Before unbinding a Button click event, call `list_button_click_events` and pass the returned index and full event signature to `unbind_button_click_event`; save and reopen to confirm the event count and target method behavior.',
  '   - For a linked prefab Button, verify the scene instance retains its `clickEvents` override after save and reopen while the prefab asset remains unchanged; use real preview input to verify handler behavior.',
  ...
  '- If `edit_prefab_json` is used, target a verified prefab path and the smallest exact JSON path or literal replacement. Inspect its returned reference validation, then reopen the prefab when the editor state allows it.',
...
const COCOS_UI_SKILL_NAME = 'cocos-mcp-kit-ui-composition';
const COCOS_UI_SKILL_TITLE = 'Cocos MCP Kit UI Composition';
...
function buildLegacyCocosMcpProjectSkillContent(options = {}) {
  const instructions = [
    ...
    '- Use focused tools when they are better primitives: `find_asset_by_name`, `list_assets`, `inspect_asset_dependencies`, `validate_asset_dependencies`, `run_script_diagnostics`, `get_script_diagnostic_context`, and screenshot tools.',
